import type { DashboardInfo } from "@context/types/dashboard"
import { Table } from "antd"
import type { ColumnsType } from "antd/lib/table"

type ErpSyntheticProps = {
  erpSyntheticList: DashboardInfo["pending_synthetic_erp"]
}
...
export function ErpSynthetic({ erpSyntheticList }: ErpSyntheticProps) {
  ...
  const erpSyntheticData: ErpSynthecticData[] = erpSyntheticList.map((erpSynthetic, index) => ({
    key: index,
...
type ErpAnalyticProps = {
  erpAnalyticList: DashboardInfo["pending_analytic_erp"]
}
...
export function ErpAnalytic({ erpAnalyticList }: ErpAnalyticProps) {
  ...
  const erpAnalyticData: ErpAnalyticData[] = erpAnalyticList.map((erpAnalytic, index) => ({
    key: index,
